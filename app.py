...
def apply_theme(url):
    st.markdown(f"""
        ...
        .result-card h1, .result-card p, .result-card h3 {{ color: #000000 !important; font-weight: bold !important; }}
...
# --- الصفحة الأولى: (تغيير رابط الخلفية لسفينة في البحر) ---
if st.session_state.page == 'welcome':
    # رابط مباشر لصورة سفينة حاويات في البحر
    apply_theme("https://cdn.pixabay.com/photo/2017/08/01/21/04/ship-2568077_1280.jpg")
    st.markdown("""
        ...
        with st.spinner('جاري التحليل...'):
            ...
            st.markdown(f"""
                <div class="result-card">
                    ...
                    <p>وقت الوصول المقدر: {travel_time} ساعة</p>
                </div>
            """, unsafe_allow_html=True)

            st.table(pd.DataFrame({
                "العنصر": ["المسافة", "الوزن", "نوع الشاحنة", "نوع البضاعة"],
                "القيمة": [f"{dist} كم", f"{wght} كغ", truck, c_type]
            }))

            # التقييم بـ 5 نجوم فقط
            st.write("---")
